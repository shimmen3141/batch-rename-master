...
これは復元作業そのものの教訓である。**凍結側の手順を戻すだけでは足りず、戻した各stepが現在のplatformで実行可能かを、projectが後から記録した事実に照らして確認する必要がある**。凍結側の手順は、その事実が判明する前に書かれていることがある。修正では、跨ぎ選択に「最近タブ/検索から試す」技法と「できなければその旨だけ教えてほしい(自動testで検査済み)」というfallbackを付けた(当初は「確認不能で構いません」と書いたが、人間へ定型語彙を要求しない方針に合わせて後の是正で言い換えた)。あわせて、reviewが指摘した内部用語(REQ-ID、handle)、UI文言のずれ、desktopの種類指定漏れ、変数依存のcleanupと不可視なfixture pathも直した。
