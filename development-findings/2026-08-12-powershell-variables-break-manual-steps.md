...
2026-08-12に実施者から具体的な理由の報告があった。**手順を上から実行すると、変数を定義したterminalでemulatorやappを起動することになる。** 起動したまま確認するには、必然的に別のterminalでコマンドを実行することになる。つまりこの種の手順では「同じwindowで続けて実行する」という前提が**そもそも成立しない**。注意書きが効かなかったのは実施者の運用の問題ではなく、手順の構造がappの起動と両立していなかったためである。
...
`004:T10`と`005:T05`の手順にも同じ構造が残っている。どちらも実施済みのtaskなので**先回りして直さない**(下記の位置づけ参照)。再実行が必要になった時点で、変数を除いてから渡す。ASDD pluginのdry-run条文への追加は未対応。

## manual文書の位置づけ(2026-08-12、人間の指示)

**manualはその時点での確認のための文書であり、将来のための過度なメンテナンスは不要である。** 追加実装でUIも機能も変わりうるので、同じ手順が後で通用する保証は無い。

Agentは「文書は正しく保たれるべき」と考えて、実施済みtaskの手順まで先回りして整備しがちなので、境界を明示しておく。

- **やる**: これから実施する手順を、実施者が実行できる状態にする。実施前に検査項目が落ちていれば戻す(移行時の欠落がこれに当たる)。実施中に判明した経路や不備をその場で反映する。
- **やらない**: 実施済みtaskの手順を、将来の再実行を想定して磨く。UIが変わる前提の記述を先回りで一般化する。
- 再実行が必要になったときは、そのときのUIに合わせて書き直す。過去の手順は「そのcommitで何を確認したか」の記録として読む。
