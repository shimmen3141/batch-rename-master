...
### 第2の原因: 証拠がある状態で、必須でない修正のためにcodeを触った

順序の問題だけでは説明が足りない。独立reviewの指摘で実際に証拠を失効させた`lib/`変更は、**P1の修正ではなかった**。

- attempt 5のP1は「testの判別力」で、その修正は`modified_time_test.dart`だけで完結する。**test変更はmanual証拠を失効させない。**
- `lib/`のcatch節を変えたのは、同じreviewのP2(例外の捕捉範囲、errorCodeの分類)への対処である。**必須ではない裁量的改善だった。**

つまり順序の是正は必要条件ではあっても十分条件ではない。reviewを先行させても、その後に別の裁量改善を入れれば同じことが起きる。

**規律**: manual証拠を取得したあとにcodeを触るのは、P0/P1に限る。P2はそのcommitでは触らず、後続taskへ送るか、証拠を取り直す判断とセットで行う。「ついでに直す」が最も高くつく局面である。

今回のP2(例外の全捕捉化)は、REQ-016が破れる経路を塞ぐ実質的な改善だったので、結果として直した価値はあった。しかし**その判断は「証拠を取り直す」というコストを見積もったうえで下すべき**で、筆者は見積もらずに直した。
...
- **manual証拠取得後のcode変更はP0/P1に限る**(上記「第2の原因」)。P2は後続へ送るか、証拠の取り直しコストを見積もったうえで一緒に判断する。
...
以後、筆者は実装を含むtaskで**独立reviewをmanualより先に**通し、**manual証拠取得後のcode変更をP0/P1に限る**。

第2の原因は2026-08-12の最終review(attempt 7)で指摘され、この節を追記した。当初の分析は原因を順序へ一元化しており、切り分けが1段浅かった。
