...
**独立reviewは連鎖させてよい(差分review)。** 同じtaskの2回目以降のreviewは、全範囲を読み直さず、**前回reviewしたhead以降の差分**だけを対象にする。manual確認の指摘や決定の変更で直すたびに全範囲を読み直すと、直した点と無関係な記録の古さまで毎回拾い、FAILの回数だけが増えるためである。

- 最初のreviewは`base..head`の全範囲を見る。FAILでも、**確認できた点と指摘を分けて**記録へ残す(次回はそれを前提にする)。
- 2回目以降は`前回のhead..今回のhead`を見る。reviewerは差分の**hunkではなく、差分が触ったfile全体**を読み、前回までの範囲との食い違いを確かめる。前回の指摘が閉じたかも確かめる。`lib/`・`test/`・`tool/`・依存・build設定に差分があれば、full regressionを流す。
- **記録だけの差分**(`specs/`・`development-findings/`・docsだけで、上のcode側のpathに差分が無い)で、**P2/P3の指摘を閉じるだけ**のものは、再reviewを起動しない。所有Agentが閉じた指摘と対応commitを`SELF-CHECK`としてtaskへ記録すれば足りる。
- 次のときは差分reviewにせず、全範囲を見直す: baseを取り込んで競合を解いた、仕様・contract・権限・データ保護の判定を変えた、差分が前回PASSした判定そのものを書き換えた、plan完了のreview。
- **auto-merge条件2の「exact rangeの独立PASS」は、`base..head`を切れ目なく覆う連鎖で満たす。** 各区間がPASSか、FAIL区間なら後続の区間でその指摘が閉じたことが確認されていること、最後の区間がPASSか上の`SELF-CHECK`であることを要する。task.mdのreview記録へ各区間のrangeと判定を並べる。
- 差分reviewも1回のreviewとして数える。FAILの累計(3回で`blocked`)はtask単位のまま変えない。
...
人間へ返す選択肢を現在の路線の改良だけで構成しない。「この問題を別の枠組みで解くとどうなるか」を1案入れる。**「そもそもここまでやる必要があるか」も選択肢に入れる。**見つからない場合は、この経緯を知らない相手へ一般的な解き方を尋ねることを人間の作業として提案する。外から得た案はそのまま採らず、前提が噛み合うかをケース表で確かめる。

指摘は重大度とは別の軸で、**成果物の欠陥**と**安全網の穴**へ分ける。分類と根拠を指摘ごとに書く。書けないものは安全網の穴として扱う。

- **成果物の欠陥**: 実装、仕様、証拠、記録が現に誤っている。実際の振る舞い、承認済みの保証、記録の真偽が対象である。P0/P1が一つでもあればFAILにする。完成度の問題であることを理由にPASSへ格上げしない。**P2/P3だけならPASSとし**、指摘は所有Agentが直すか、直さない理由を記録する(P2だけでFAILにすると、記録の細部で`blocked`まで数えてしまう)。
- **安全網の穴**: 実装は正しいが、将来の変更を検出できない。次を**すべて**満たすときだけFAILにする。
  1. 守る対象がcomposition rootから到達する**製品経路に載っている**
  2. 通り抜ける失敗が**データ損失、無断置換、偽の成功、権限逸脱、互換性破壊**のいずれかである
  3. **このprojectのCIで閉じられる**(実機、別OS、別archを必要としない)

満たさない安全網の穴は、引き受け先のtask IDを添えて残余riskとしてtaskへ記録し、受容する。受容はreviewerではなくtask所有Agentが記録する。

同じtaskで、**成果物の欠陥が無く安全網の穴だけのFAILが2回続いたら、3回目の独立reviewを起動しない。**その時点の穴を残余riskとして記録し、受容した一覧を人間へ**報告する**(判断を仰ぐのではなく報告する)。上の3条件に当たる穴があれば、それだけを直す。

CIで実行できない領域(実機、別OS、別arch、外部service)を含むtaskは、着手時に**machine検証する範囲**と**引き受け先のtask**を`task.md`へ宣言する。宣言の外側の指摘は安全網の穴として扱う。

独立reviewが足したmutationは、実装側の`tool/mutations.json`へ取り込む。対照として置いたものも落とさない。

独立reviewを走らせるmodelは、既定で**実装に使ったものより一段軽いもの**とする。照合(仕様・実装・記録の突き合わせとmutationの実行)が主で、判断より読み取りが効く作業だからである。**判定そのもの、contract、権限、データ保護に触れるtask**でだけ**実装と同等以上のもの**を使う。

**FAILが続いても上げない。** 同じところで繰り返し落ちるのはreviewerの読み取りが足りないからではなく、**解き方が合っていない**ことのほうが多い。上げるのではなく、FAILの回数で定めた手当て(同じ種類の修正を繰り返さない / 3回で`blocked`にして人間へ返す)を使う。
