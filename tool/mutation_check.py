...
MUTATIONS = [
    ("1段目成功後の包括catch除去", "lib/data/rename_exec/desktop_rename_executor.dart",
     "} catch (error) {\n      return await _rollbackAfter(temporary, handle, errorOf(error));\n    }",
     "} catch (error) {\n      return RenameFailed(errorOf(error));\n    }"),
    ("巻き戻し内のcatch除去", "lib/data/rename_exec/desktop_rename_executor.dart",
    ...
    ("主probe(目標名の実在確認)除去", "lib/data/rename_exec/desktop_rename_executor.dart",
     "if (await _probe.exists(destination)) {\n        // **`await` を落とさない。**",
     "if (false) {\n        // **`await` を落とさない。**"),
    ("UTF-8境界ループ除去", "lib/data/rename_exec/desktop_rename_executor.dart",
     "while (cut > 0 && (bytes[cut] & 0xC0) == 0x80) {\n      cut -= 1;\n    }", ""),
    ("切り詰め時のhash除去", "lib/data/rename_exec/desktop_rename_executor.dart",
     "return '${utf8.decode(bytes.sublist(0, cut))}-$digest';",
     "return utf8.decode(bytes.sublist(0, cut));"),
    ("退避失敗をnameConflictにしない", "lib/data/rename_exec/desktop_rename_executor.dart",
     "return RenameFailed(\n          _conflictWithDetail(destination, result, handle, candidate),\n        );",
     "return RenameFailed(_nativeError(result, handle, candidate));"),
    ("一時名確保失敗をnameConflictにしない", "lib/data/rename_exec/desktop_rename_executor.dart",
    ...
    ("再観測で衝突したときの巻き戻し除去", "lib/data/rename_exec/desktop_rename_executor.dart",
     "return await _rollbackAfter(\n          temporary,\n          handle,\n          RenameError(",
     "return RenameFailed(\n          RenameError("),
