...
| 2026-08-05 | 種類選択の枠組み | 読み込みの最初に**「画像 / ドキュメント / フォルダ」のような種類の選択肢**を置き、種類ごとに適した選択 UI へ分岐する(フォルダ=ファイルシステムを辿る、画像=すべて/アルバムのギャラリー)。これにより「画像にしか通用しない扱い(撮影日時など)」を実装しやすくなる。**画像経路は権限モデルが別**(MediaStore/フォトピッカーの URI は読み取り専用で、リネームには `MediaStore.createWriteRequest` の同意が要る)ため、T9 では枠だけ用意し中身は写真機能で実装する | 開発者 |
...
| T7 | 読み込み導線の再設計(蓄積の取り消し・単一導線・種類選択)の**仕様更新** → 再承認依頼 | M | T4 | pending | #81 |
...
- **残っている論点(spec の open_questions に落とし、承認前に解消する)**:
  1. **フォルダ内の一括選択ができるか**。システムのファイル選択画面(Android の DocumentsUI)に
     ...
     まとめてリネームしたい場合に手選択しかないと実用に耐えない。案: (a) 種類「フォルダ」だけ**ツリー権限**に
     して全件読み込む(旧 A の挙動を一部戻す)、(b) アプリ内に選択画面を作る、(c) 手選択のみで割り切る。
  ...
  3. **親フォルダ同一の確認が要るか**。DocumentsUI はフォルダ移動で選択が解除されるため実質フォルダ跨ぎは
     ...
     (c) 実質起きないので何もしない。
  4. **種類選択の粒度と名前**。「画像 / ドキュメント / フォルダ」の3つで足りるか(動画・音声は?)。
     「フォルダ」は実際には「ファイルシステムを辿ってファイルを選ぶ」であり、名前から
     「フォルダを丸ごと選ぶ」と誤解されないか。
