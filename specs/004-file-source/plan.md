...
| T7 | 読み込み導線の再設計(蓄積の取り消し・単一導線・種類選択)の**仕様更新** → 再承認依頼 | M | T4 | done | |
| T8 | **実機スパイク**: SAF ドキュメント URI でリネームできるかの確認(ホスト検証) | S | T4 | pending | |
| T9 | 読み込み導線の再設計の**実装**(種類選択 → 選択 UI → リスト画面) | L | T7, T8 | pending | |
