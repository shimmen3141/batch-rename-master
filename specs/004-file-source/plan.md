...
| T7 | 読み込み導線の再設計(蓄積の取り消し・単一導線・種類選択)の**仕様更新** → 再承認依頼 | M | T4 | blocked | |
...
- 受け入れ条件:
  ...
  - [ ] **種類選択の枠組み**が定義されている。少なくとも「フォルダ(=ファイルシステムを辿る)」を規範とし、
        「画像」経路は**枠のみ**(中身は写真機能)であることを境界に書く。
