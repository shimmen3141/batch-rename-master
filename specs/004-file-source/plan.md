...
| T9 | 読み込み導線の再設計の**実装**(種類選択 → 選択 UI → リスト画面) | L | T7, T8 | done | #83 |
...
- 2026-08-05 / T7 / 申し送りの回収 / verifier の非ブロッキング指摘4点も対応: discovery の「002 に追加/除去 API」→ `setFiles`/`removeFile`/`clearFiles`、決定事項の「読み込み経路」「返り順」行に取り消し線・読み替え注記、ADR-001 の「複数フォルダ蓄積の確認」→ 新導線の確認。
- 2026-08-05 / T7 / **OQ-1 を解消**(開発者の実機確認): システムのファイル選択画面の右上ケバブメニューに「**Select all**」があり、フォルダ内を一括選択できた。よって導線 B のままで成立し、ツリー権限へ戻す案 (a)・アプリ内選択画面の案 (b) は不採用。`pickFolder()` は契約に含めないことが確定し(実装コードの削除は T9)、REQ-011 から OQ-1 依存の注記を外した。**これで 004 spec の未解決事項はゼロ**になり、2仕様とも**承認可能な状態**。留保として「Select all の有無は端末依存。無い環境が見つかったら再検討」を spec に残した。
...
- 2026-08-05 / T8 / **OQ-1 は先に解消**(開発者の実機確認): システムのファイル選択画面の**右上のケバブメニューに「Select all」があり、フォルダ内を一括選択できた**。したがって導線 B のままで一括選択は成立し、ツリー権限へ戻す案 (a)・アプリ内選択画面を作る案 (b) は採らない。※この解消は 004 spec 側の記述なので **PR #80(T7)に反映**する。
...
- 2026-08-05 / T8 / PR #84 作成(Closes #82)。verifier PASS(試行1回)。補足指摘(ツリー側の生 URI)も追記済み。マージ待ちで停止。
