...
- [x] `spec.md` の Status が approved(Light)。(2026-08-05: T7 の更新を再承認)
...
- [x] `FileSource` ポート・fake・リスト操作・結線ロジックは**実 IO なし**でサンドボックス検証できる。
...
|----|--------|------|------|------|-------|
| T1 | 振る舞い仕様の作成(Light)+ 001/002 仕様更新の洗い出し | M | - | - | #51 |
| T2 | `FileSource` ポート + 元場所ハンドル + 作業セット + fake + 002 結線 | M | T1 | REQ-001, REQ-002, REQ-003, REQ-006, REQ-007, REQ-008 | #52 |
| T3 | UI 入口: フォルダを開く / ファイルを選ぶ(追加・除去、fake で結線・widget test) | M | T2, 002-file-list.T2 | REQ-007, REQ-008 | #53 |
| T4 | 実 `FileSource`(Android SAF + Windows ピッカー)+ 実データ入口配線(ホスト検証) | L | T3 | REQ-002, REQ-003 | #54 |
| T5 | 時系列ソート2種+警告の**仕様更新**(001 Strict / 002 / 004)→ 再承認依頼 | M | T2 | - | #63 |
| T6 | 時系列ソート2種+警告の**実装**(取得可否・ソート・警告表示) | M | T5 | REQ-010 | #64 |
| T7 | 読み込み導線の再設計(蓄積の取り消し・単一導線・種類選択)の**仕様更新** → 再承認依頼 | M | T4 | - | #81 |
| T8 | **実機スパイク**: SAF ドキュメント URI でリネームできるかの確認(ホスト検証) | S | T4 | - | #82 |
| T9 | 読み込み導線の再設計の**実装**(種類選択 → 選択 UI → リスト画面) | L | T7, T8 | REQ-004, REQ-005, REQ-009, REQ-011, REQ-012 | #83 |
...
### T7: 読み込み導線の再設計の仕様更新(蓄積の取り消し・単一導線・種類選択)

2026-08-05 の実機検証で、現行の導線(「フォルダを開く」で全件が自動追加・「ファイルを選ぶ」と2ボタン)が
利用者の期待と合わないことが分かったため、004 spec を書き換える。決定済みの方針は「決定事項」の
2026-08-05 の4行(蓄積の取り消し / 単一導線 B / 選別は選択段階 / 種類選択の枠組み)。

- 変更対象: specs/004-file-source/spec.md, specs/002-file-list/spec.md(必要なら specs/001-rename-core/)
- 受け入れ条件:
...
### T8: 実機スパイク — SAF ドキュメント URI でリネームできるか(ホスト検証)

**T9 の前に実施する。** 導線 B は `OPEN_DOCUMENT` で得た**ドキュメント URI**を書き戻し先にするが、
そのURIで実際に改名できるかは文書では確定できない(`DocumentsContract.renameDocument` は対象が
`FLAG_SUPPORTS_RENAME` を持つ場合のみ成功する)。ここが崩れると導線 B ごと作り直しになるため、
**1ファイルだけ改名する最小の確認**を先に行う。

- 変更対象: 使い捨ての確認用コード(本実装には残さない)。結果は plan の作業ログと 004 spec / ADR へ反映
- 受け入れ条件:
...
### T9: 読み込み導線の再設計の実装(種類選択 → 選択 UI → リスト画面)

- 変更対象: lib/ui/file_source/, lib/data/file_source/, lib/ui/file_list/, lib/main.dart, test/spec_004_file_source/, test/spec_002_file_list/
- 受け入れ条件:
