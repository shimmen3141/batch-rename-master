...
| T2 | `FileSource` ポート + 元場所ハンドル + 作業セット + fake + 002 結線 | M | T1 | REQ-001, REQ-002, REQ-003, REQ-006, REQ-007, REQ-008 | done | #52 |
| T3 | UI 入口: フォルダを開く / ファイルを選ぶ(追加・除去、fake で結線・widget test) | M | T2, 002-file-list.T2 | REQ-007, REQ-008 | done | #53 |
| T4 | 実 `FileSource`(Android SAF + Windows ピッカー)+ 実データ入口配線(ホスト検証) | L | T3 | REQ-002, REQ-003 | done | #54 |
| T5 | 時系列ソート2種+警告の**仕様更新**(001 Strict / 002 / 004)→ 再承認依頼 | M | T2 | - | done | #63 |
| T6 | 時系列ソート2種+警告の**実装**(取得可否・ソート・警告表示) | M | T5 | REQ-010 | done | #64 |
| T7 | 読み込み導線の再設計(蓄積の取り消し・単一導線・種類選択)の**仕様更新** → 再承認依頼 | M | T4 | - | done | #81 |
| T8 | **実機スパイク**: SAF ドキュメント URI でリネームできるかの確認(ホスト検証) | S | T4 | - | done | #82 |
| T9 | 読み込み導線の再設計の**実装**(種類選択 → 選択 UI → リスト画面) | L | T7, T8 | REQ-004, REQ-005, REQ-009, REQ-011, REQ-012 | done | #83 |
