...
1. 「＋ 変更する名前を設定する」を押し、「元の名前」と、リテラル「_ok」を追加してシートを閉じます。
   ...
   - 確認: 下部バーのボタンが2行の表示(「命名ルール」+ 設定中のルール + 右端に「編集」)に戻り、実行ボタンが押せるようになってラベルが `1 件をリネーム` になる。**設定中のルールは `[元の名前]_ok` のようにトークンを並べた形で出る**(`008:T20`。説明文にはならない)。
...
3. もう一度実行button(`N 件をリネーム`)を押し、今度は何も押さずに6秒待ちます。
