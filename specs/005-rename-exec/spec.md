...
| 20c | 連番の桁が足りないルール | 桁不足を提示する。**対象ファイルを持たないので、ファイルごとの提示にはならない** | REQ-009 |
| 20d | 変更後名が重複する 2 件 | **その 2 件それぞれについて、種別が「重複」であることが一覧を見た状態で分かる**(「警告がある」だけでは足りない) | REQ-009 |
| 20e | 全件の提示を、ファイルごとの警告からも一覧全体の件数表示からも開く | **同じ内容**が読める | REQ-009 |
