...
| 7 | 巻き戻し期限を過ぎてから巻き戻しを要求 | 何も起きない(そもそも提示されない) | REQ-007, SM-001 |
...
| 判定不能語 | 「適切に」「必要に応じて」「可能な限り」を規範部分から排除。唯一残る量的な未定は巻き戻し期限の長さで、これは未解決事項として明示している。 |

## 未解決事項

承認前に解消する。

1. **巻き戻し期限の長さ**(既定の秒数)。参考デザインのトーストは時限表示で、5〜10 秒程度が一般的。
2. **停止時に一時名が残った場合、その1件だけ元の名前へ戻すことを試みる**(REQ-005)扱いでよいか。「巻き戻さない」という決定とは整合していると考えている(一時名は成功した改名ではなく、未完了の内部手段であり、利用者が意図した名前でもない)が、**唯一の巻き戻し**なので明示の確認を求める。
