...
| 20 | 作成日時が不明なファイルに、作成日時トークンだけのルール | 行には**結果だけ**(名前が空になり改名されない)を出し、**原因**(どのトークンの基準日時が取れないか)は**ルールを変える操作と同じまとまりへ一度だけ**出す | REQ-009, REQ-021 |
