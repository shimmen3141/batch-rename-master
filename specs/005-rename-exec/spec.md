...
| 20 | 作成日時が不明なファイルに、作成日時トークンだけのルール | 行が出すのは **(i) 変更後名が空になること** と **(ii) そのファイルが改名の対象にならないこと** の2つだけ。**原因**(どのトークンの基準日時が取れないか)は**ルールを変える操作と同じまとまりへ一度だけ**出す | REQ-009, REQ-021 |
| 20a | 例20 の状態でファイルが 30 件 | 原因の提示は**1つ**である(30 回繰り返さない) | REQ-021 |
| 20b | 例20 の状態で、空名どうしが同じ名前になる | **行に重複の警告を出さない**(そのファイルは改名されないため)。詳細には出す | REQ-021 |
| 20c | 連番の桁が足りないルール | 桁不足を**ルールを変える操作と同じまとまり**へ出す。**どの行にも出さない**(対象ファイルを持たない警告のため) | REQ-009 |
| 20d | 変更後名が重複する 2 件 | **その 2 行が**「変更後名が他と重複する」ことを提示する(無地の警告印だけでは足りない) | REQ-009 |
| 20e | 行の警告を選ぶ / 一覧全体の件数表示を選ぶ | **同じ詳細**が開き、全件と説明が読める | REQ-009 |
| 20f | 警告 0 件 | 警告を提示しない。「問題なし」の表示は出してよい | REQ-010 |
| 20g | rule=[元名][日時 作成]、作成日時が不明なファイルが 30 件(**名前は空にならない**) | **30 行とも「日時トークンが空になる」ことを提示する**。**どのトークンかは行に出さず、まとまりに1つ**(30 回繰り返さない)。REQ-021 規則1は適用しない | REQ-009, REQ-021 |
| 20h | 例20g で、行の警告と件数表示のどちらから詳細を開いても | どの 30 件が対象かと、トークンの説明の両方が読める。**まとまりにも同じ説明が出ていること自体は要求違反ではない** | REQ-009 |
...
- 警告・確認・結果の**文言と UI 部品**。色は `AppColors` のセマンティック色を用いること自体は UI 側の規約。
  **配置は revision 7.0 で要求になった** — REQ-009 が**警告の種別ごとに、行・まとまり・詳細へ何を載せるかの表**を課す。**行はトークンを名指ししない**(名指しはまとまりと詳細)。どの部品でそれを作るかは自由のままである。
...
| 観点 | 結果 |
|---|---|
| 提示場所を要求にした(008:T15) | 提示方法をすべて「自由とする点」へ置いた結果、実装は一覧上部の集約帯1か所に全件を並べた。**008:T07 が 360dp で計測し、重複 30 件のうち固定 132px の内訳に 2 件しか入らないことを確かめた**(1 件が 4 行へ折り返して 68px)。folder を跨ぐ重複は通常経路で起きるので、件数が多いのは例外ではない。原因は情報の置き場所で、文言は「file 名・場所・変更後名だけが行ごとに違い、残りは全行で同一」だった。**「自由」が実装の裁量ではなく仕様の穴になっていた**ため、場所だけを要求へ引き上げた(UI 部品は自由のまま)。あわせて `DigitShortageWarning` が対象ファイルを持たない(=行へ帰属しない)ことが分かり、行・まとまり・詳細の 3 分割が導かれた。 |
