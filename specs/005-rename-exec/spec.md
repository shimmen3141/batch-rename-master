...
| 20d | 変更後名が重複する 2 件 | **その 2 行から**重複が読み取れる | REQ-009 |
| 20e | 行の警告を選ぶ / 一覧全体の件数表示を選ぶ | **同じ詳細**が開き、全件と説明が読める | REQ-009 |
| 20f | 警告 0 件 | 警告を提示しない。「問題なし」の表示は出してよい | REQ-010 |
| 20g | rule=[元名][日時 作成]、作成日時が不明なファイルが 30 件(**名前は空にならない**) | **30 行とも**そのファイルが影響を受けていることを提示する。**トークンの説明はまとまりに1つ**(30 回繰り返さない)。REQ-021 規則1は適用しない | REQ-009, REQ-021 |
| 20h | 例20g で、行の警告と件数表示のどちらから詳細を開いても | どの 30 件が対象かと、トークンの説明の両方が読める | REQ-009 |
...
- 警告・確認・結果の**文言と UI 部品**。色は `AppColors` のセマンティック色を用いること自体は UI 側の規約。
  **配置は revision 7.0 で一部が要求になった** — REQ-009 (a)(b)(c) が「どのファイルが影響を受けるかは行から読み取れる」「なぜ起きるかの説明は一度だけ・ルールを変える操作と同じまとまり」「行からも件数表示からも詳細を開ける」を課す。**(a) と (b) は粒度が違い、同じ警告について両方が同時に成り立つ。** どの部品でそれを作るかは自由のままである。
