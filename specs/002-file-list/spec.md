...
| REQ-002 | must | `setSortMode(name/createdAt/modifiedAt/size)` は該当キーで昇順・安定ソートし、`sortMode` を更新する。時系列は**作成日時（UI ラベル「作成日時」）と更新日時（同「更新日時」）の2種**を提供する。作成日時ソートでは、**作成日時が不明な item を末尾へ回し**（不明どうしの相対順は元の順を保つ）、判明している item を昇順に並べる。 | VER-001 |
...
| REQ-011 | must | `sortMode` が作成日時のとき、**作成日時が不明な item の件数**を警告として供給する（0 件なら警告を供給しない）。判定は**ファイル種別ではなく取得可否**で行う（画像でも EXIF 撮影日時を持たない場合があるため）。作成日時以外のソートでは供給しない。 | VER-001, VER-002 |
...
| 8 | 作成日時 [b=2024, a=2023, c=不明] を作成日時ソート | items=[a,b,c]（不明の c は末尾） | REQ-002 |
| 9 | 上の状態で作成日時ソートを選択 | 「作成日時を取得できないファイルが 1 件」の警告を供給 | REQ-011 |
| 10 | 全件の作成日時が判明している状態で作成日時ソート | 警告は供給しない | REQ-011 |
| 11 | 全件の作成日時が不明な状態で作成日時ソート | 全件の警告を供給し、順序は元の相対順を保つ（失敗しない） | REQ-011, REQ-012 |
| 12 | 上の状態で更新日時ソートへ切替 | 警告は供給されず、`modifiedAt` 昇順に並ぶ | REQ-002, REQ-011 |
...
| ID | 種別 | 成果物パス | 対象 |
|---|---|---|---|
| VER-001 | unit | test/spec_002_file_list/controller_test.dart | REQ-001〜012 |
| VER-002 | widget | test/spec_002_file_list/file_list_view_test.dart | REQ-003, REQ-004, REQ-010, REQ-011（ウィジェット操作→状態反映・場所サブ情報・作成日時ソート時の警告表示） |

## 反証ログ

| 観点 | 結果 |
|---|---|
| 仕様を見ずに書いた例との照合 | 代表例12件を状態遷移と照合。例3で「連番は選択行のみ・表示順」を 001 の generatePreview に委ねる形に整理（重複定義を避けた）。例8〜12（作成日時ソートと警告）を 004 T5 で追加し、不明の並び順・警告の有無・更新日時への切替を網羅。 |
| 弱すぎ（ズルい実装） | REQ-006 を「001 の generatePreview に一致」と接続し、変更後名を空返しする実装を排除。REQ-002 に安定・昇順を明記し空ソートを排除。REQ-008 に「既存ハンドルは追加しない・追加分は選択」を明記し、全追加/全無視/未選択追加の実装を排除。REQ-011 に「0 件なら供給しない・作成日時以外では供給しない」を明記し、常に警告を出す/常に出さない実装の双方を排除。 |
| 強すぎ（過剰な条件） | 内部表現・視覚を「自由とする点」で明示的に解放。ソート方向・名前順の詳細は open_questions に送り固定しすぎない。警告の UI 表現（文言・提示方法）と、作成日時の取得経路は本仕様で縛らない（004 の関心）。 |
| 異常系の網羅 | 空 files（行データ空）、全未選択（プレビュー空）、reorder の端インデックスを VER-001 で扱う。004 T5 で「作成日時が一部不明」「全件不明」を追加（REQ-002 の末尾回し・REQ-011 の警告・REQ-012 の継続動作）。 |
...
- **作成日時が不明な item の扱い**: 作成日時ソートでは**末尾**へ回す（不明どうしは元の相対順を保つ。REQ-002）。処理は失敗しない（REQ-012）。
- **警告（REQ-011）**: 作成日時ソートが選択されているとき、**作成日時が不明な件数**を警告として供給する（0 件なら供給しない）。判定は**ファイル種別ではなく取得可否**で行う。
