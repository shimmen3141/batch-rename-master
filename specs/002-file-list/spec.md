...
- Status: approved <!-- 2026-08-01 承認 / 2026-08-04 004 由来の更新と VER 成果物指定の変更を開発者が再承認 -->
...
| REQ-013 | must | 各行データは、その item の**作成日時（不明の場合はその旨）と更新日時の双方**、および**作成日時が不明か**を判別できる情報を供給する。行 UI はこれを用いて、どの item の作成日時が不明か（=更新日時で代替されたか）を提示できる。**提示方法（警告マーク・強調色など）は視覚デザインであり本仕様の対象外**（`AppColors` のセマンティック色を用いること自体は UI 側の規約）。 | VER-001, VER-002 |
...
| 観点 | 結果 |
|---|---|
| 仕様を見ずに書いた例との照合 | 代表例13件を状態遷移と照合。例3で「連番は選択行のみ・表示順」を 001 の generatePreview に委ねる形に整理（重複定義を避けた）。例8〜13（作成日時ソート・警告・行の識別）を 004 T5 で追加し、代替キーでの並び・警告の有無・更新日時ソートとの一致（例11↔12）・行データの供給を網羅。 |
| 弱すぎ（ズルい実装） | REQ-006 を「001 の generatePreview に一致」と接続し、変更後名を空返しする実装を排除。REQ-002 に安定・昇順を明記し空ソートを排除。REQ-008 に「既存ハンドルは追加しない・追加分は選択」を明記し、全追加/全無視/未選択追加の実装を排除。REQ-011 に「0 件なら供給しない・作成日時以外では供給しない」を明記し、常に警告を出す/常に出さない実装の双方を排除。REQ-002 の代替キーを「当該 item の更新日時」と特定し、固定値・0・現在時刻を代替に使う実装を排除（例11↔12 の順序一致で観測可能）。REQ-013 で「双方の日時＋不明フラグ」を要求し、不明を隠して作成日時だけ見せる実装を排除。 |
