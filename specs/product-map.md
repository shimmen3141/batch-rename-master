...
| 読み込み画面の状態復元 | 「すべて」を開き直したときに、前回の場所を開き、前回選んだfileを選択済みにする | `013:T07`の実機確認で開発者が挙げた(2026-08-25)。**本人が「ふとした思い付き」と明示**しており、良い解き方かは未判断。004 REQ-004(蓄積しない)と両立するかを先に確かめる — 復元は「前回の選択を残す」ではなく「選び直しの初期値」である必要がある。**2026-09-18に位置づけが変わった** — `008:T03`が行のcheckboxを廃止して「一覧＝rename対象」にしたため、**除去は×で1手だが、戻すにはfolderごと選び直すことになる**(取り消しは直後の1回だけ。002 REQ-017)。この候補は**便利機能ではなく、その非対称を埋める導線**になった。**2026-09-18に `008:T26` として起票した** — 以後は plan と task が正本で、この行は出所の記録である |
...
| app内file browserの範囲選択 | folder内の全選択と、長押し+dragでの範囲選択を加える | `008:T07`の実機確認で開発者が挙げ、2026-09-21に`008:T36`(仕様) / `T37`(実装)へ昇格した。004 REQ-020をT36で承認済み。入口の定義と近道の取りやめ(`T11`/`T12`)とは別の論点である |
| app内file browserのテキストpreview | browserの行でテキストfileの中身(先頭)を見せる | `013:T07`の実機確認のU4に含まれていたが、`008:T13`で開発者が**画像・動画だけ**に絞った(2026-09-24)。行の枠が狭く先頭数文字しか読めない。入れるなら先頭何byteを読むか、binaryを誤ってtextとして出さない判定、004の「判定を新設しない」方針との関係を先に決める |
