...
| ルールをchipのUIで見せる | 下部バーのルール設定buttonに、`[元の名前][01…]` の字面ではなく実際のtokenのchipを並べる。警告の詳細でも「3番目のトークン」ではなく説明文へchipを埋め込む | `008:T20`の実機確認で開発者が挙げた(2026-09-16)。**本人が「将来的には」と書いている。** 下部バーは`T20`、警告詳細は`T19`が作った形の上に載る。狭幅でchipが収まらないときの扱いを先に決める |
