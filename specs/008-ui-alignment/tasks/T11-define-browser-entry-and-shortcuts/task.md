...
### 2026-08-29 に受け取った入力(このtaskにはまだ入れていない)

`T07`の実機確認で開発者が挙げた。**004 specへ要求を足す変更**なので、このtaskの範囲へ
入れるかは**planの判断**である。入れるならこのtaskの「決めること」へ移し、入れないなら
[`product-map.md`](../../../product-map.md)の将来候補として残す。

- **folder内のfileを全選択するcheckbox。** 現状は1件ずつ触るしかない。004 REQ-016
  (選択の確定)との関係と、folderを跨いだときの意味を決める必要がある。
- **長押し + dragでの範囲選択**(一般的なfileアプリにあるもの)。**入力手段の追加**であり、
  desktopやaccessibilityでの代替が要る。単独taskに値する大きさである。
