...
**変更したfile**: `specs/004-file-source/spec.md`(REQ-015 / REQ-020 / 代表例22→22・22b・22c・22d / 代表例29 /
自由とする点 / `013:T03`由来の記録・Play policy節・`008:T36`由来の記録への追記 / Status行 / 「008:T11 由来の更新」節)、
`specs/008-ui-alignment/plan.md`(対象・全体の受け入れ条件・人間の決定を3行追加)、
`T12`の`task.json`の`covers`と`task.md`。**`lib/`と`test/`に差分は無い。**
