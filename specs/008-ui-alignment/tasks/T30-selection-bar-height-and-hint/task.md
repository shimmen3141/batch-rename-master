...
## 独立review attempt 4(2026-09-19)

- `dev...bc85cd7` — **FAIL**(Sonnet。開発者の指示。`.worktrees/008-T30-review4`)。
  **これで合計3回目のFAILであり、`AGENTS.md` に従って `blocked` にして人間へ返す。**
  - **P1(成果物の欠陥)**: **横向き・低い高さ + 大きい文字倍率で、下へ回した吹き出しが
    画面の下端からはみ出す。** `_flipIfClipped` は「**上**に置いたときに上端からはみ出すか」
    だけを見ており、**下へ回した結果が下端に収まるかを一度も確かめていない**。
    reviewer の実測: 732×360(横向き)× 倍率3.0 で `bottom = 492.5 > 360`、
    800×320 × 倍率2.0 でも `bottom = 321 > 320`。閉じる操作の円は画面内に残るが、
    **「ファイルは削除されません」という誤解防止の本文が画面の外へ出る。**
    `AndroidManifest.xml` は `orientation` と `fontScale` を `configChanges` に
    宣言しているので、横向きも最大文字サイズも**対象の構成である**(推測ではない)。
    **attempt 2 の P1 とまったく同じ実害の型**が、軸を変えて再発した。
  - **P3(安全網の穴・受容。引き受け先 `T30`)**: `M381`(`IgnorePointer` を上向きのときだけ
    効かないようにする)が SURVIVED。**いまは上向きの吹き出しの下に押せる部品が無い**
    ので実害は無い(モード中の帯は空箱、ケバブと × は吹き出しの外)。将来そこへ操作が
    増えたときに気づけない穴として記録する。
  - **P3(記録)**: `tool/mutations.json` の **`M356` が欠番**で、理由がどこにも無い。
    → **`M356` は `proxyDecorator` の指定そのものを消す対照として書いたが、
    Dart の構文にならず(`null ?? (…)`)投入前に外した。** ここに記録する。
  - reviewer が**問題無しと確かめた点**: **attempt 3 の P1 は閉じている**
    (`IgnorePointer` の範囲は狭すぎも広すぎもせず、閉じる操作は生きている)。
    `ignoringSemantics` は既定なので**読み上げから文言は消えていない**
    (`find.bySemanticsLabel` で実測)。閉じる操作の tooltip も健在。
    attempt 2 の P2(死んだ定数の罠)は解消済み。`M359` / `M360` / `M373` / `M378` / `M379`
    の等価性は**全件回帰で再確認**でき、記録は正しい。`T29` と attempt 1〜3 が閉じた保証、
    `T32` はいずれも健全。

### mutation の生出力(attempt 4)

```
command: flutter test test/spec_002_file_list test/spec_004_file_source
M381 | SURVIVED | IgnorePointer を下向きのときだけ効かせる(いまは実害なし。受容)
M382 | SURVIVED | no-op guard(reviewer のハーネス健全性確認。指摘ではない)
...
M378 | SURVIVED
M379 | SURVIVED
5 mutations: 0 KILLED, 5 SURVIVED, 0 SKIPPED
```

## 解き方を洗い直す(2026-09-19)

**3回の FAIL はすべて同じ源から出ている** — 吹き出しが**任意の画面寸法 × 文字倍率に対して
自分の置き場所を解かなければならない浮いた箱**であることである。

| 回 | 壊れ方 | 直し |
|---|---|---|
| attempt 2 | 上端からはみ出して本文が消える | 収まらなければ下へ回す |
| attempt 3 | 下へ回すと重なった行のtapを吸う | 飾りを `IgnorePointer` で素通しに |
| attempt 4 | 下へ回すと**下端**からはみ出して本文が消える | (ここで止めた) |

**保証そのものを問い直す。** いま両立させようとしているのは次の2つである。

1. **ツノがアイコンに接する**(受領した要望。位置を1点に固定する)
2. **どの画面・どの文字倍率でも本文が全部見える**(`008` が一貫して守ってきた基準)

箱の高さは文字倍率で伸びるので、**空いている余白より箱が大きくなった時点で1と2は両立しない。**
いまの路線はその境界を条件分岐で埋め続けており、埋めるたびに別の軸が出ている。
**1を弱める(ツノを縦方向の自由と引き換えにする)と、この型の不具合は消える。**

## 解き方を変えた(2026-09-19。開発者の判断)

**開発者から「上に固定しつつ、はみ出る場合は表示しない、のように極端な条件を逃がすのはどうか」
という案を受領し、これを採った。**(あわせて「下端からはみ出すとはどういう状況か、
なぜ本文が消えるのか」を問われたので、実測で答えた。下の表。)

### なぜ消えるのか(旧実装の実測。2026-09-19)

**下の表は `setSurfaceSize` で測ったもので、rect そのものは正しい**(layout の制約は
動いていた)。**ただし当時の「収まる/はみ出す」の判定は `MediaQuery` を読んでおり、
そちらは動いていなかった**(独立review attempt 5 の P1)。作り直し後の実測は下の
「実測(2026-09-19。`tester.view` で画面を変えて測り直した)」を正とする。

箱の幅を 232 に固定しているので、**文字倍率が上がると折り返しが増えて縦に伸びる**。

| 画面 | 倍率 | 箱の高さ | 置いた位置 | 画面外 |
|---|---|---|---|---|
| 320×800 | 1.0 | 59 | y 60〜119(アイコンの上) | なし |
| 320×800 | 3.0 | **284** | y 234〜518(下へ回した) | なし |
| 732×360(横向き) | 1.0 | 59 | y 60〜119 | なし |
| 732×360 | 2.0 | 134 | y 187〜321 | なし |
| **732×360** | **3.0** | **284** | y 208.5〜**492.5** | **下へ 132.5** |

横向きの画面の高さは 360 しかないので、アイコンの下(y=208.5)から 284 伸ばすと下端を
132px 超える。**見えるのは上半分だけで、切れて消えるのは後半の
「ファイルは削除されません。」の側**である。誤解を防ぐ一文が、いちばん残したい場面で切れていた。

### 新しい作り

- **置き場所はアイコンの上の1つだけ。** 上下左右を**まとめて1回**判定し、
  安全領域に収まらなければ**取り下げる**(場所を探し直さない)。
- **判定が済むまで透明にしておく。** 測る前の位置がちらつかない。
- `IgnorePointer` は残す。**いまの置き場所の下に押せる部品は無い**が、
  性質そのものを hit test で直接見る test を置いた。
- ツノの向き・箱とツノの並び順・向きのリセットは**対象のコードごと消えた**ので、
  対照 `M375` / `M376` / `M377` / `M381` を表から外した(置き場所が1つになったため)。

**引き換えに、文字を大きくしている利用者には補足が出なくなる。** いちばん助けが要る設定で
出ないという逆説だが、**いまはその設定で「切れて半分しか見えない」**ので悪化はせず、
...
- `dev...5ee30d7` — **FAIL**(Sonnet)。**実装ではなく検証方法の誤りである。**
  - **P1(成果物の欠陥・記録)**: 幅を変えたと書いた widget test が、**実際には幅を
    変えていなかった**。`tester.binding.setSurfaceSize` は **layout の制約は変えるが
    `MediaQuery.of(context).size` は既定(800×600)のまま**で、吹き出しの収まり判定は
    `MediaQuery` を読むため**格子が一度も条件を変えていなかった**。
    自分でも最小再現で確認した(`MediaQuery.size = 800×600` / `LayoutBuilder = 360×800`)。
    → `tester.view.physicalSize` / `devicePixelRatio` / `padding` で画面を変える
    `_setScreen` を置き、**格子を回し直して下の表を実測値へ更新した**。
    projectに広がっている書き方なので
    [`development-findings/2026-09-19-set-surface-size-does-not-move-media-query.md`](../../../../development-findings/2026-09-19-set-surface-size-does-not-move-media-query.md)
    へ残した(**既存の利用箇所は layout の検査なので壊れていない**)。
  - **P3(安全網の穴)**: 横方向の収まり判定を無効化しても落ちない(P1 の裏返しである)。
    → **幅200dpの test を足して閉じた**。表へ取り込むときに `M385` を振った
    (reviewer は `M386` と呼んでいたが、その番号は別の対照に使った)。
  - reviewer が**問題無しと確かめた点**: **「収まらないなら出さない」の中身は健全**
    (正しい方法で測り直しても、普通の文字では必ず出て、大きい文字では必ず出ない)。
    取り下げの経路で `Timer` / `OverlayEntry` が残らない。**「透明にしてから見せる」の
    判定前フレームは外から観測できない**(2段の `addPostFrameCallback` が同じ
    `drawFrame` の中で完結する)。`IgnorePointer` の範囲と semantics も正しい。
    `T29` とこれまでの保証、`T32` はいずれも健全。表から外した4件の理由も実体と一致。

### 直したこと(attempt 5 の後)

- **安全領域の取り方を変えた。** reviewer の指摘を直す途中で、
  **左右と下の inset まで要求すると、横向き + ジェスチャーナビ(右に 40 程度の inset)の
  端末で普通の文字でも一度も出なくなる**ことが実測で分かった。この app は画面の端まで
  使う作りで、ヘッダのケバブも外すアイコンも inset の内側へ寄せていない。
  **上(status bar)だけを数え、左右と下は画面そのものを境にする。**

## 実測(2026-09-19。`tester.view` で画面を変えて測り直した)

**出るのは文字倍率1.5まで**で、**1.6以上では幅にも高さにもよらず出ない**(上端を超えるため)。
境目は独立review attempt 6 が 240通りの格子で詰めた。下の表は粗い格子である。

| 画面 | 倍率1.0 | 倍率1.3 | 倍率2.0 | 倍率3.0 |
|---|---|---|---|---|
| 320×800 | 出る | 出る | **出ない** | **出ない** |
| 411×800 | 出る | 出る | **出ない** | **出ない** |
| 732×360(横向き) | 出る | 出る | **出ない** | **出ない** |
| 800×800 | 出る | 出る | **出ない** | **出ない** |
| 200×800(狭すぎ) | **出ない** | — | — | — |

箱の高さは倍率1.0で59、1.3で71、2.0以上ではヘッダの上に残る余白(約120)を超える。

**これは開発者に知っておいてほしい事実である** — Android の文字サイズを最大付近まで
上げている利用者には、**補足が一度も出ない**。実機確認の手順4で境目を見てもらい、
常用の設定で出ないようなら**全幅の帯**へ作り替える(本文が1〜2行で済むので大きい文字でも収まる)。

## 独立review attempt 6(2026-09-19)

- `dev...d70e29b` — **PASS**(Sonnet)。**指摘0件**(P0/P1/P2 いずれも無し)。
  - reviewer が**機械で確かめた点**: `flutter test` 938 / `analyze` / `format` /
    `check specs` が記録と一致。表 371件の find が厳密に1回ずつ一致。
    **`tester.view` が `MediaQuery` を本当に動かすこと**と `setSurfaceSize` が
    動かさないことの両方を自作 probe で確認(attempt 5 の P1 は閉じている)。
  - **幅 × 高さ × 文字倍率 × inset の 240通りを測り直した**:
    **出るかどうかは幅にも高さにもよらず文字倍率だけで決まり、1.5 までは必ず出て、
    1.6 以上では必ず出ない。** 箱の高さも記録どおり(倍率1.0 で59、1.3 で71)。
    **「いつも出さない」への退化は無い**(240通りすべてで倍率1.0 なら出た)。
  - **安全領域の取り方は app 全体と整合している**: `main.dart` は `SafeArea` で
    包んでおらず、ヘッダのケバブも外すアイコンも inset の内側へ寄せていない。
    横向き + ジェスチャーナビ相当の inset(右40・下48・上24)でも**普通の文字なら出る**。
    箱と閉じる操作の右端が inset 領域へ出るのは**`T30` 以前からの app 全体の方針**で、
    ここで持ち込んだ劣化ではない。
  - 受容した7件(`M359` / `M360` / `M373` / `M378` / `M379` / `M384` / `M386`)を
    **独立に再実行して記録どおりであることを確認**した。
  - **P3(安全網の穴・受容。引き受け先 `T30`)**: **下端の収まり判定は到達しない**
    (`M388` SURVIVED。全件でも確認)。置き場所をアイコンの上の1点に決めたので、
    箱の下端が画面の下端へ届く経路が無い。**将来また下へ回すなら生き返る分岐**なので
    残し、対照を表へ取り込んだ。
  - **P3(記録)**: 横方向の対照の番号が本文と表で食い違っていた(本文 `M386` / 表 `M385`)。
    → **本文を表へ揃えた。**

### mutation の生出力(attempt 6 の対照)

```
command: flutter test
M388 | SURVIVED | 下端の収まり判定だけを無効化する(到達しない。受容)
1 mutations: 0 KILLED, 1 SURVIVED, 0 SKIPPED
```

## manual確認の結果(2回目。2026-09-19)

対象commit: **`cd29a23`**(`lib/` の最終変更は `94474e4`。以後 `lib/` は動かしていない)。
開発者の報告: 「**確認事項についてはすべて問題なかった。**」

| 手順 | 結果 |
|---|---|
| 1 がたつき(出入りで縦に動かない・場所の表示幅が戻った) | **成立** |
| 2 重ねる吹き出し(ツノが近い・継ぎ目の線が無い・原文の文言・円+×が欠けず押せる・×で即消え・3秒でフェード・モード解除で即消え) | **成立** |
| 3 掴んだ行(触れた瞬間に色と振動・ドラッグできる・離すと戻り鳴らない・選択色と別) | **成立** |
| 4 文字サイズ最大 | **成立。ただし機械の実測と食い違う**(下記) |
| 5 横向き | **成立。ただし縞模様を観測**(下記) |

### 機械の実測と食い違った点(手順4・5)

**実機では文字サイズ最大でも吹き出しが出た。** 機械の格子では倍率1.6以上で出ないので、
**Android の文字サイズ設定の最大が倍率1.5 程度までで、test で使った 2.0 / 3.0 に
実機の設定では届かない**ためと考えられる。**これは未確認の推測である** —
確かめるならディスプレイサイズ設定と併用する。

**「収まらないなら出さない」が実機で発火していない**ということなので、
**この判定は実機では保険として働いている**。機械の格子(倍率 2.0 / 3.0)では
確かに発火するので、検査としては生きている。

### 横向きで観測した縞模様

**横向き × 文字サイズ最大で画面の下に縞模様**(Flutter の overflow 表示)が出た。
**吹き出しは正常に出ている**ので、出どころは別の widget である(下部の帯が有力だが未特定)。

開発者の判断: 「**横向きのフォントサイズ最大など考慮できるものではない。そもそも横向きを
想定する必要もないと思う。今回はこのままでよい**」。
→ **`T35`(新設)**が「横向きを対象にするか(縦固定にするか)」を引き受ける。

### あわせて受領した吹き出しの詰め

ツノをやや左へ / 全体をもう少し下へ / 文言を1行へ / フェードまでを5秒へ /
**枠だけシアンで中は黒**(ツノと箱の境界線は引き続き見せない)。
→ **`T34`(新設)**が引き受ける。**`T30` の欠陥ではない**(確認事項はすべて成立している)。
