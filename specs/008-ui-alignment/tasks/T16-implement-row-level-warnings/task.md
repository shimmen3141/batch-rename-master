...
作ったのは**警告の詳細を見る**ための提示である。**同じmodalに二つのtaskが手を入れない**よう、
入れ物はこのtaskが持ち、`T14`は文言だけを後から合わせることにした。**2026-09-02に、この分担の
相手は`T19`(警告の詳細modalを作り直す)へ移した** — `T14`の`task.md`にも書いてある。
...
- Next Agent action: **無し。このtaskは完了している。** 続きは`T17`(警告をファイル単位へ一本化する提示を定義する)から始める
...
**仕分けと引き受け先は2026-09-02に確定した。** 承認済みの正本に触れるもの(要望7は
**002 REQ-015** と 005 spec の例20c、要望2は 005 spec の例20e、要望1は **002 REQ-006**
(行の変更後名が`generatePreview`に一致)と **005 REQ-019** の境界)は、`plan.md`の方針どおり
**仕様更新task`T17`と実装task`T18`/`T19`/`T20`へ分けた**。**Agentが自分で仕様を緩めない。**

| 要望 | 引き受け先 |
|---|---|
| 1(`(変更なし)`と実行可否) | `T17`(仕様) + `T18`(表示) + `T20`(実行可否) |
| 2(行からの詳細はその行だけ) | `T17`(仕様) + `T19` |
| 3・4(modalの重複・冗長さ) | `T19` |
| 5(警告の文言。「連番の桁不足」「作成日時不明」「更新日時不明」・`・`区切り) | `T19`(行とmodalの文言)。**要望5の前半はルール設定button上の表示を指している** — `T17`がルール単位の警告を廃止すれば`T20`が表示ごと外して消える。**`T17`がルール単位の警告を残す判断をした場合は、button側の文言を`T20`が引き受ける** |
| 6・7(警告をファイル単位へ一本化・色・桁不足を行へ) | `T17`(仕様) + `T18` + `T20` |
| 8(行の右上へ寄せる対象) | `T18`。原文の主語が一意に決まらなかったので**2026-09-02に開発者へ確認し、対象は「行の警告」と確定した** |
| 9(ルール設定buttonの形) | `T20` |
| 10・13(区切り線・上部見出し) | `T10` |
| 11・12(場所の提示・読み込み導線) | `T08` |
| 14(実行buttonの文言) | `T20` |
...
1. PR #161 を ready 化して merge した(merge commit `b5ac61e`)。
2. `dev`上の結果とCIを確認し、`task.json`を`done`にした。
3. 改善要望14件を `T17`〜`T20`・`T08`・`T10`へ登録した(上の表)。
4. `T18`のmanualへ、行の警告のtap範囲を実機で見る項目を入れた。
5. `T19`へ、件数と詳細の数え方を固定するassertionを置く要求を入れた(R-A SURVIVEDの引き受け先)。

**このtaskに残っている作業は無い。** 続きは`T17`から始める。
...
| risk | 引き受け先 |
|---|---|
| **行の警告のtap範囲を実機で見ていない**(manual確認をemulatorで行ったため。attempt 7 が挙げ、開発者が2026-09-02に受容) | `008:T18` |
