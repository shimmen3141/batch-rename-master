...
- `docs/design/Bulk Renamer.html` の行の`warnText` / `rowBg` / `cardBorder` と、
  ヘッダの`warnLabel`。**適用する画面範囲は一覧の行とヘッダの警告表示に限る** —
  下部の実行バーは`005:T09`の成果なので動かさない。
...
- 集約帯の置き換え(ヘッダの件数と、詳細のトグルまたはmodal)。
  ...
  **帯を廃止するならこの論点は消える。残す形を選ぶなら上限の基準を一覧の取り分へ変える**
  こと(基準の決定は`T15`が持つ)。
...
下部バー(`_RenameActionBar`)へ**警告を載せる部分だけ**をこのtaskが持つ。実行buttonの
振る舞い、ルール設定sheetの開き方、バーの構成そのものは動かさない(`005:T09`の成果)。
参考designはルール設定buttonの中に「命名ルール」の見出しと設定中のルールの2行を持ち、
見出しの右にスペースがある。**開発者はそこへ出す案を挙げた**(2026-08-29)。土台として扱い、
離れる場合は理由をここへ書く。余白・typographyは`T10`、modalの文言は`T14`。
