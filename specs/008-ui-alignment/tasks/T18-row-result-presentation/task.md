...
`rowWarningIconInkNudge` が比例する `rowWarningFontSize` は**コンパイル時定数 11 であり、
利用者の文字倍率(`textScaler`)に追随しない**。`Icon` も `applyTextScaling` が既定 false で
拡大しない。一方 `Text` の `fontSize` は倍率で拡大するので、**倍率を上げるほど字面の中心の
差が開く**。独立review attempt 4 の実測:

| `textScaler` | icon size | gap(アイコンの中心 − 文字の中心) |
|---|---|---|
| 1.0 | 11.0 | **1.18** |
| 1.3 | 11.0 | **2.37** |
| 2.0 | 11.0 | **4.30** |
...
**倍率 2.0 で、このtask自身が「下がりすぎ」と定義した上限 2.5 を超える。**
`textScaler` を設定するwidget testはこのwidgetに1件も無い。**扱いは人間の判断待ちである**
(下の「独立review attempt 4」)。

| 手順 | 対象commit | 結果 |
|---|---|---|
| 1(緑と赤)/ 4(`（変更なし）`) | `e5aceed` | 成立。以後 `_NewName` と `rowWarningsOf` / `rowWarningLabel` は不変 |
| 2′ のA〜C・E・F / 3′(押しやすさ) | `176d311` | 成立。以後動いたのはアイコンの縦位置だけ |
| 2′ の確認D — 確認A(揃って見える) | `9c2d6df` | **成立** |
