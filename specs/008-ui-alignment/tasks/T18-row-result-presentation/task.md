...
「枠の外の空白」はいま無反応である。**この残余riskは手順3′で閉じ直す。**
`AGENTS.md` は「manual証拠は対象commit以後にcodeが変わったら再利用しない」としている。

**縮んだこと自体は開発者が承認した**(2026-09-03)。原文は「**枠の中だけが反応でよいです。
むしろそっちの方が自然です。**」**見た目と当たり判定が一致する形が選ばれた**ので、
行全体を押せるようにする案・枠の余白を増やす案は採らない。手順3′の確認Dは、
狭さを問うのではなく**枠の外が反応しないことを確かめる**項目である。
...
「再確認(2026-09-03。2回目)」。**開発者の言葉をそのまま引用する。**

| 手順 | 結果 | 開発者の記述(原文) |
|---|---|---|
| 2′ 警告の見た目 | **D以外は成立** | 「D以外は全て確認できた。」 |
| 2′ 確認D(！マークの高さ) | **不成立** | 「Dは、画像のようにまだわずかに！が上にずれているように見える。テキストが枠に対して少しだけ下に位置しているのに対し、！は枠の中央に位置しているためにずれているように見える。文字の性質によるものに見えるし、そこまで違和感はないので対応しなくても良いが、簡単に対応できるなら対応してほしい。」 |
| 3′ 押しやすさ(実機) | **成立** | 「全て確認できた。」 |

**`008:T16` から引き受けた残余risk(行の警告のtap範囲)は閉じた。** 手順3′の確認A〜Dが
実機で成立した — 枠の中を指で押して開き、少し外側でも入り、選択が誤爆せず、
**枠の外は反応しない**(2026-09-03 の決定どおり)。

**添付されたスクリーンショットは読めていない**(Windows側のpathで、AI containerから
到達できない)。**判断は開発者の記述だけに基づく。**

#### 確認Dの原因(開発者の見立ては正しい)

`CrossAxisAlignment.baseline` で**baselineは揃っている**(box の中心どうしの差は 0.14px。
`row_result_display_test.dart` が固定している)。ずれて見えるのは**字面(ink)の位置**である。

- **Material icons** は em box いっぱいに描かれ、baseline の上 1em を占める。
  → ink の中心は **baseline − 0.5em**。
- **CJKの字面**は baseline の上 0.88em 〜 下 0.12em あたりを占める。
  → ink の中心は **baseline − 0.38em**。

差は **0.12em**(11px なら約 1.3px)で、アイコンのほうが**上**になる。**box を揃えると
ink がずれる**という、文字の性質による差である。開発者の「文字の性質によるものに見える」
という見立てと一致する。

**「対応しなくても良いが、簡単に対応できるなら」という条件付きの依頼である。**
**断定へ強めない。** 対応の可否は下の「確認Dへの対応」に書く。
