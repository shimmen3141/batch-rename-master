...
- T11/T12: 保存場所1件時の入口、近道の見分け、戻る矢印の向き、空folder表示を持つ。T38はそれらを複製せず、T39がT12の結果と整合するよう導線・配置を決める。
...
- T11/T12との重複と実装順を整理し、T39へ観測可能なwidget testと実機確認を引き渡す。
...
- Evidence revision: 2026-09-22会話、dev@bef8337（T37 codeは別worktree）。
- Waiting for: なし。
- Requested action: なし。
- Next Agent action: T11/T12およびT37の最新状態を照合し、状態表と仕様変更案を作る。物質的な選択だけ一問ずつ開発者へ尋ねる。
