...
| 2026-08-29 | 桁不足の置き場所 | 連番の桁不足は 001 の `DigitShortageWarning` が**対象fileを持たない**ルール単位の警告なので、行へは出さず**ルール設定の導線と同じまとまり**へ出す。**実行は止めない** — 001の生成規則では数字が伸びるだけで名前は失われず、強制実行を選べば`autoResolve`が桁をそろえる。止めると001 REQ-011の桁拡張が使われる経路が無くなる | 開発者 |
| 2026-08-29 | 空名の出し方 | 空名は**原因がルール側・結果がfile単位**なので分ける。行は**(i)変更後名が空になること (ii)そのfileが改名の対象にならないこと**の2つ、原因はルール設定の導線へ一度だけ。**Androidでは作成日時が取れないため、作成日時トークンだけのルールで全件が空名になる**(現在は同じ原因をfile件数ぶん繰り返している) | 開発者 |
| 2026-08-29 | 集約帯 | **廃止**し、ヘッダーの件数(`⚠ N 件の問題`)とmodalへ移す。modalは**ヘッダーからも行の警告文からも**開く。一覧の常時の占有が0になり、`T07`の残余risk N-9(帯が一覧の半分以上を覆う)が消える | 開発者 |
