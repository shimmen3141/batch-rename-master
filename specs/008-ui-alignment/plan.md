...
- **`covers`は仕様更新taskが埋める。** 全taskの`covers`は現在空である。REQ IDが確定するのはT01/T03/T05/**T11**/**T15**の承認時なので、その5taskの受け入れ証拠に「対応する実装taskの`covers`を書く」ことを入れてある。T07/T08/T10/**T13/T14**は仕様を変えないため、空のままが正しい。
