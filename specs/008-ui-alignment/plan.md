...
| 2026-08-29 | 002/005 の再承認(**1回目。取り直しが必要**) | **002 REQ-015(行データが警告を持つ)と 005 contract revision 7.0(REQ-009の提示場所3条件・REQ-021のまとめ規則2つ・REQ-010)を承認。** 判定・自動解決・実行の振る舞いは不変。**この承認後に独立reviewが2回FAILし、REQ-009を種別ごとの表へ書き換えた**(承認時の文より弱くなる修正を一度入れてしまったため)。**承認を取り直す** | 開発者 |
