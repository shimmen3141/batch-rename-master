...
| 2026-08-29 | ソート文脈の警告 | 開発者が「並び順をプルダウンにすると右にスペースが空くので、そこへ作成日時ソート時の警告を出す**のはどうでしょうか**」と案を示した(原文)。`T15`はこれを受けて**002 REQ-011を行へ移さない**と判断した(行が示すのは*そのfileの値が無い*こと、REQ-011が示すのは*一覧全体の並び順が別のkeyで決まっている*ことで粒度が違う)。**配置は`T01`が持つ** | Agent(開発者の案に基づく) |
| 2026-08-29 | 002/005 の再承認 | **002 REQ-015(行データが警告を持つ)と 005 contract revision 7.0(REQ-009の提示場所3条件・REQ-021のまとめ規則2つ・REQ-010)を承認。** 判定・自動解決・実行の振る舞いは不変 | 開発者 |
