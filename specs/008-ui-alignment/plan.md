...
| 2026-09-02 | `T16`のmanual確認と改善要望 | **`T16`はここで閉じ、同時に受領したUIの改善要望14件は新taskへ分ける。** manual確認は5項目すべて成立し、`T07`から引き受けた残余risk N-9 は閉じた。要望のうち**警告をファイル単位へ一本化する案(桁不足を該当行へ、ルール単位の警告を廃止)と、行から開く詳細をその行だけにする案**は、承認済みの005 contract revision 8.0(例20c・例20e)と衝突するため、**仕様更新task`T17`と実装task`T18`/`T19`/`T20`へ分ける**。要望11・12は`T08`、要望10・13は`T10`が引き受ける | 開発者 |
