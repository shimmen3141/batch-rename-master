...
| 2026-08-14 | preflightの所有判定 | **2段階作成 + 空の回収の両方を採る**(方針決定)。ただし**この方針を書き上げた本文はまだ承認されていない** | 開発者方針決定 |
...
**ただし`spec.md`は現在`draft`である。** 2026-08-14の再承認後にREQ-011(preflightの所有判定)を変更したため、その版の再承認を待っている。**`plan.md`のこの表は「承認した論点」の記録であって、`spec.md`本文が承認済みであることを意味しない。** 状態の正本は[`spec.md`](spec.md)の`Status`である。
