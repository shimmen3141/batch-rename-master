...
```text
=== 判定 ===
A) RENAME_NOREPLACE が効いている。
```

プログラムは3つのcaseを実行します。**case B(フラグ無し)が「成功して上書きした」ことが重要**です。フラグ有りだけを見ても、フラグが効いたのか、そのpathがそもそも上書きを拒むのかが区別できないためです。

**出力を全部コピーして報告してください。**

`B)` `C)` `D)` `E)` のいずれかが出た場合は、**この時点で報告して止めてください**。手順5へ進む必要がありません。
...
ただし**`A)`以外になる可能性が十分にあります。それがこの確認の目的です。** どの結果でも「失敗」ではありません。

続けて、`/sdcard`がどのfilesystemかを記録します。「FUSEを経由している」ことを推測ではなく観測にするためです。
...
**期待**: `Type:`に`fuseblk`や`fuse`が出る、または`mount`の行に`/mnt/user/0/emulated`や`fuse`が現れる。**出力をそのまま報告してください。** 判定はこちらでします。
...
1. 手順4の出力(全部)
2. 手順5の出力(全部)。`stat -f` / `mount` の出力も
3. 使ったデバイス(実機かemulatorか、Androidのバージョン、CPU)
4. うまくいかなかった手順があれば、そのコマンドとエラー

**判定はこちらで行います。** プログラムが出す `A)`〜`E)` はあくまで目安なので、**生の戻り値とerrnoをそのまま送っていただければ十分です。**
