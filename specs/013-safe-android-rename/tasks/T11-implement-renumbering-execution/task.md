...
`AGENTS.md`は[`docs/design/Bulk Renamer.html`](../../../../docs/design/Bulk%20Renamer.html)を005 UIの**配置・導線・情報階層**の正本とし、対象taskへ適用画面範囲の明記と照合を求める。

- **適用する画面範囲**: **実行結果のtoast**(`file_list_view.dart`の結果SnackBar)。他の画面は触らない。
- **design側の該当箇所**: 1行のtoast(✓アイコン + 本文 + undoボタン)。
...
- **契約を優先する。** designは配置・導線・情報階層の正本だが、**判定・データ保護・時間制約はdesignへ移さず契約とtestを正本とする**(`AGENTS.md`)。ここは「利用者が確認していない名前になった事実を隠さない」というデータ保護側の要求である。
...
- Review attempt 7: `691d3f5..17fc284` — FAIL — P1×3、P2×4。**設計そのものは今回も正しい方向**と確認された。**私が申告した4つのmutationはすべて再現された。**
  1. **P1: 1段目を通ったあとの例外で、巻き戻さずに抜けていた。** 実体は一時名にあるのに理由文に名前が出ない。**attempt 6で新設した回帰testが、この経路を通しながら`isA<RenameFailed>()`しか見ておらず、自分が作り出した残骸状態を1行も観測していなかった。** 同じrangeで登録したばかりのOQ-007も**すでに実装より狭かった**。→ 例外時も巻き戻し、`_rollbackAfter`へ集約した。
  2. **P1: 生存名の第2要素(未実行の目標名)にtestが無かった。** 除去しても387件すべてPASS。**先行fileが後続fileの確認済み目標名を横取りし、`x (1) (1).jpg`が確定する** — attempt 1のP1-1で直した症状が無言で再発しうる状態だった。→ (2)と(3)のtestを追加した。
  3. **P1: attempt 6のP1-5(handoff)が解消されていなかった。** 更新されたのは`Next Agent action`の1行だけだった。**作業記録には解消済みと書いていた。**
  - P2×4も解消(`covers`が空である理由、生存名の第3要素のtest、findingの改善結果、PR本文のmutation件数)。

- Review attempt 8: `691d3f5..29e9d5d` — FAIL — P1×2、P2×3。attempt 7のP1×3は解消と確認された。
  1. **P1: 実在を肯定的に検出した目標名へ、no-replaceフラグだけを頼りにrenameしていた。** 1段目のあとに再観測が無く、**フラグを黙って無視する環境では2段目が成功して、実在を確認済みの別の実体を上書きする**。**REQ-025の存在理由そのものがその環境なのに、`exists`と分かった側だけ塞げていなかった。** → **1段目のあとに目標名をもう一度観測する。** 判定は増やさず、観測するだけである。
  2. **P1: `return await`を守るtestが失われ、mutation申告が事実と違った。** attempt 7で例外処理を内側へ移した結果、回帰testが内側のcatchを通るようになり、**外側の`await`を検証しなくなった**。**確認漏れの4回目。**
  - P2×3も解消(別folderの改名要求、一時名確保ループの2分岐、OQのowner表記)。

- Review attempt 9: `691d3f5..e1ea812` — FAIL — P1×3、P2×2。**申告した11種のmutationはすべて再現された。** 再観測の分岐そのものも正しいと確認された。
  1. **P1: 退避と巻き戻しに実在確認が無かった。** portが呼ぶrenameは3本あるのに、attempt 8で`destination`にだけ再観測を足していた。reviewerが実FSで2件再現した — **(a) 前回の異常終了で残った一時名を退避が黙って上書きする**(その残骸は実装自身が「利用者が直せる」として受容範囲に置いている)、**(b) 退避と巻き戻しの間に他processが元の名前を作ると、巻き戻しがその実体を上書きする**。→ 退避先と巻き戻し先にも`_probe.exists`を通した。巻き戻せないときは戻さず、現在の名前を理由へ出す。
