...
- **専用subfolderとmarker fileの生成・検査・削除**(REQ-011)。**一時名で作りmarkerを書き終えてから正規名へrenameする**(所有の確立を不可分にする)。**空のmarkerless残骸は回収する。**
...
- subfolder名の規則(正規名と一時名)。**利用者が見て正体が分かること**と衝突しにくさを両立する。
...
- `spec.md`のVER-007が挙げる5項目(正常・異常終了で残らない / markerを持つ残骸を片付ける / **一時名の空の残骸を回収する** / **markerが無く空でもない同名実体では削除せず中止する** / **markerを観測対象にしない**)をtestで検査する。**片付ける側と中止する側は逆向きなので両方要る。**
