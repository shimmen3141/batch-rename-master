...
- **契約をrevision 5へ更新する範囲。** `T11`が決着させたOQ-001(試行上限=8)とOQ-005(生存名の一時名を「計画が使う一時名すべて」へ広げる)は、**実装が契約より広い状態**で`dev`に入っている。`T10`が自分のOQを決めた時点で、**両者をまとめてrevision 5として契約へ戻す**(人間の承認が要る)。
