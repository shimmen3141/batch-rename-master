...
現状では、**読み込んでいないfileと同じ名前になる改名を、実行前に検出できない**。005 contract revision 4はこれをREQ-004・REQ-011で塞ぐと定めたので、占有名を供給する経路が要る。
...
- **`validate`だけでなく`autoResolve`も占有名を避ける**(005 contract REQ-011)。警告だけを占有名で出して解決を占有名抜きで行うと、確認した目標名が占有名と衝突したまま実行へ渡る。
