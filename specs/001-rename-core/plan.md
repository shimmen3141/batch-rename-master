...
| T2 | ドメインモデル + 単純トークン評価(元名・自由テキスト) | M | T1 | pending | |
| T3 | 連番・日時トークンの評価 | M | T2 | pending | |
| T4 | プレビュー生成(選択・並び順反映、連番の割り当て) | M | T3 | pending | |
| T5 | ドライラン検証(重複・桁不足の警告生成) | M | T4 | pending | |
| T6 | 自動解決(強制実行時の名前確定) | M | T5 | pending | |
