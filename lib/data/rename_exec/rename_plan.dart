...
/// 001 の `autoResolve` が保証するのは**最終**状態の一意性だけなので、
/// `a→b, b→c` のような入れ替えは順序で解く。順序の入れ替えだけでは解けない
/// 循環(`a→b, b→a`)がある場合に限り、一時名への改名を挿入して解く。
///
/// 実ファイルには触れず、`package:flutter` にも `dart:io` にも依存しない
/// (CON-001)。
RenamePlan planExecution(List<RenameRequest> requests) {
  // 各要求が「今どの名前を持っているか」。実行の進行に合わせて更新する。
  final current = <RenameRequest, String>{
    for (final r in requests) r: r.originalName,
  };
  // 一時名が元の名前・目標名・既に発行した一時名のいずれとも衝突しないようにする。
  final usedNames = <String>{
    for (final r in requests) ...[r.originalName, r.targetName],
  };
  ...
  bool blocked(RenameRequest r) => current.entries.any(
    (e) => !identical(e.key, r) && e.value == r.targetName,
  );
  ...
  while (pending.isNotEmpty) {
    ...
    for (final r in pending) {
      final wanted = pending.any(
        (o) => !identical(o, r) && o.targetName == current[r],
      );
      if (wanted) {
        blocker = r;
        break;
    ...
    final temporary = _temporaryName(current[blocker]!, usedNames);
...
String _temporaryName(String currentName, Set<String> usedNames) {
