...
- **モバイル幅（スマホ）**: ファイルリスト全面 + 下部のルール設定ボタン（「命名ルール」＋設定中のルールの2行、右端に「編集」）→ ボトムシートでトークン編集。**ボタン全体が一つの押下対象**で、「編集」は飾り（`008:T20`）。設定中のルールは `[元の名前][01…]` のようにトークンを並べた形で出る。
- **デスクトップ幅（≥840dp・タブレット/横向き）**: 左にリスト、右にルールビルダーの 2 ペイン。
- チェックボックス選択・ソート切替（名前/作成日時/サイズ/カスタム）・行のドラッグ並び替え・トークンの追加(エディタで「追加」を押したとき)/タップ編集(「確定」を押したとき)/削除が、その場でプレビュー（変更後名）へ反映される。
