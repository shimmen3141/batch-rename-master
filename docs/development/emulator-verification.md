...
- **005 リネーム実行**: unit固有のfixture、操作、期待結果、記録する証拠は[`development-units/complete-rename-execution/manual-verification.md`](../../development-units/complete-rename-execution/manual-verification.md)を参照する。この文書にはemulator・desktop buildの起動やbranch確認など、複数unitで共通するhost手順だけを残す。
